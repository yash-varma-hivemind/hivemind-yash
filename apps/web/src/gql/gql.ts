/* eslint-disable */
import * as types from './graphql';
import { TypedDocumentNode as DocumentNode } from '@graphql-typed-document-node/core';

/**
 * Map of all GraphQL operations in the project.
 *
 * This map has several performance disadvantages:
 * 1. It is not tree-shakeable, so it will include all operations in the project.
 * 2. It is not minifiable, so the string of a GraphQL query will be multiple times inside the bundle.
 * 3. It does not support dead code elimination, so it will add unused operations.
 *
 * Therefore it is highly recommended to use the babel or swc plugin for production.
 * Learn more about it here: https://the-guild.dev/graphql/codegen/plugins/presets/preset-client#reducing-bundle-size
 */
type Documents = {
    "query OnboardingRoles {\n  onboardingRoles\n}\n\nquery OnboardingAdminSteps($role: String!) {\n  onboardingAdminSteps(role: $role) {\n    id\n    role\n    title\n    body\n    order\n    createdAt\n    updatedAt\n  }\n}\n\nmutation CreateStep($input: CreateOnboardingStepInput!) {\n  onboardingCreateStep(input: $input) {\n    id\n    role\n    title\n    body\n    order\n    createdAt\n    updatedAt\n  }\n}\n\nmutation UpdateStep($input: UpdateOnboardingStepInput!) {\n  onboardingUpdateStep(input: $input) {\n    id\n    role\n    title\n    body\n    order\n    createdAt\n    updatedAt\n  }\n}\n\nmutation DeleteStep($id: ID!) {\n  onboardingDeleteStep(id: $id)\n}\n\nmutation ReorderSteps($input: ReorderOnboardingStepsInput!) {\n  onboardingReorderSteps(input: $input) {\n    id\n    role\n    title\n    body\n    order\n    createdAt\n    updatedAt\n  }\n}": typeof types.OnboardingRolesDocument,
    "query OnboardingPage {\n  onboardingMySteps {\n    id\n    role\n    title\n    body\n    order\n    completed\n    completedAt\n  }\n  onboardingMyProgress {\n    completed\n    total\n    percentage\n  }\n}\n\nquery ManagerTeamProgress {\n  onboardingTeamProgress {\n    userId\n    name\n    role\n    completed\n    total\n    percentage\n    enrolled\n  }\n}\n\nmutation CompleteStep($stepId: ID!) {\n  onboardingSetStepComplete(stepId: $stepId) {\n    id\n    completed\n    completedAt\n  }\n}\n\nmutation IncompleteStep($stepId: ID!) {\n  onboardingSetStepIncomplete(stepId: $stepId) {\n    id\n    completed\n    completedAt\n  }\n}\n\nmutation AssignRole($input: AssignOnboardingRoleInput!) {\n  onboardingAssignRole(input: $input) {\n    userId\n    name\n    role\n    completed\n    total\n    percentage\n    enrolled\n  }\n}": typeof types.OnboardingPageDocument,
};
const documents: Documents = {
    "query OnboardingRoles {\n  onboardingRoles\n}\n\nquery OnboardingAdminSteps($role: String!) {\n  onboardingAdminSteps(role: $role) {\n    id\n    role\n    title\n    body\n    order\n    createdAt\n    updatedAt\n  }\n}\n\nmutation CreateStep($input: CreateOnboardingStepInput!) {\n  onboardingCreateStep(input: $input) {\n    id\n    role\n    title\n    body\n    order\n    createdAt\n    updatedAt\n  }\n}\n\nmutation UpdateStep($input: UpdateOnboardingStepInput!) {\n  onboardingUpdateStep(input: $input) {\n    id\n    role\n    title\n    body\n    order\n    createdAt\n    updatedAt\n  }\n}\n\nmutation DeleteStep($id: ID!) {\n  onboardingDeleteStep(id: $id)\n}\n\nmutation ReorderSteps($input: ReorderOnboardingStepsInput!) {\n  onboardingReorderSteps(input: $input) {\n    id\n    role\n    title\n    body\n    order\n    createdAt\n    updatedAt\n  }\n}": types.OnboardingRolesDocument,
    "query OnboardingPage {\n  onboardingMySteps {\n    id\n    role\n    title\n    body\n    order\n    completed\n    completedAt\n  }\n  onboardingMyProgress {\n    completed\n    total\n    percentage\n  }\n}\n\nquery ManagerTeamProgress {\n  onboardingTeamProgress {\n    userId\n    name\n    role\n    completed\n    total\n    percentage\n    enrolled\n  }\n}\n\nmutation CompleteStep($stepId: ID!) {\n  onboardingSetStepComplete(stepId: $stepId) {\n    id\n    completed\n    completedAt\n  }\n}\n\nmutation IncompleteStep($stepId: ID!) {\n  onboardingSetStepIncomplete(stepId: $stepId) {\n    id\n    completed\n    completedAt\n  }\n}\n\nmutation AssignRole($input: AssignOnboardingRoleInput!) {\n  onboardingAssignRole(input: $input) {\n    userId\n    name\n    role\n    completed\n    total\n    percentage\n    enrolled\n  }\n}": types.OnboardingPageDocument,
};

/**
 * The graphql function is used to parse GraphQL queries into a document that can be used by GraphQL clients.
 *
 *
 * @example
 * ```ts
 * const query = graphql(`query GetUser($id: ID!) { user(id: $id) { name } }`);
 * ```
 *
 * The query argument is unknown!
 * Please regenerate the types.
 */
export function graphql(source: string): unknown;

/**
 * The graphql function is used to parse GraphQL queries into a document that can be used by GraphQL clients.
 */
export function graphql(source: "query OnboardingRoles {\n  onboardingRoles\n}\n\nquery OnboardingAdminSteps($role: String!) {\n  onboardingAdminSteps(role: $role) {\n    id\n    role\n    title\n    body\n    order\n    createdAt\n    updatedAt\n  }\n}\n\nmutation CreateStep($input: CreateOnboardingStepInput!) {\n  onboardingCreateStep(input: $input) {\n    id\n    role\n    title\n    body\n    order\n    createdAt\n    updatedAt\n  }\n}\n\nmutation UpdateStep($input: UpdateOnboardingStepInput!) {\n  onboardingUpdateStep(input: $input) {\n    id\n    role\n    title\n    body\n    order\n    createdAt\n    updatedAt\n  }\n}\n\nmutation DeleteStep($id: ID!) {\n  onboardingDeleteStep(id: $id)\n}\n\nmutation ReorderSteps($input: ReorderOnboardingStepsInput!) {\n  onboardingReorderSteps(input: $input) {\n    id\n    role\n    title\n    body\n    order\n    createdAt\n    updatedAt\n  }\n}"): (typeof documents)["query OnboardingRoles {\n  onboardingRoles\n}\n\nquery OnboardingAdminSteps($role: String!) {\n  onboardingAdminSteps(role: $role) {\n    id\n    role\n    title\n    body\n    order\n    createdAt\n    updatedAt\n  }\n}\n\nmutation CreateStep($input: CreateOnboardingStepInput!) {\n  onboardingCreateStep(input: $input) {\n    id\n    role\n    title\n    body\n    order\n    createdAt\n    updatedAt\n  }\n}\n\nmutation UpdateStep($input: UpdateOnboardingStepInput!) {\n  onboardingUpdateStep(input: $input) {\n    id\n    role\n    title\n    body\n    order\n    createdAt\n    updatedAt\n  }\n}\n\nmutation DeleteStep($id: ID!) {\n  onboardingDeleteStep(id: $id)\n}\n\nmutation ReorderSteps($input: ReorderOnboardingStepsInput!) {\n  onboardingReorderSteps(input: $input) {\n    id\n    role\n    title\n    body\n    order\n    createdAt\n    updatedAt\n  }\n}"];
/**
 * The graphql function is used to parse GraphQL queries into a document that can be used by GraphQL clients.
 */
export function graphql(source: "query OnboardingPage {\n  onboardingMySteps {\n    id\n    role\n    title\n    body\n    order\n    completed\n    completedAt\n  }\n  onboardingMyProgress {\n    completed\n    total\n    percentage\n  }\n}\n\nquery ManagerTeamProgress {\n  onboardingTeamProgress {\n    userId\n    name\n    role\n    completed\n    total\n    percentage\n    enrolled\n  }\n}\n\nmutation CompleteStep($stepId: ID!) {\n  onboardingSetStepComplete(stepId: $stepId) {\n    id\n    completed\n    completedAt\n  }\n}\n\nmutation IncompleteStep($stepId: ID!) {\n  onboardingSetStepIncomplete(stepId: $stepId) {\n    id\n    completed\n    completedAt\n  }\n}\n\nmutation AssignRole($input: AssignOnboardingRoleInput!) {\n  onboardingAssignRole(input: $input) {\n    userId\n    name\n    role\n    completed\n    total\n    percentage\n    enrolled\n  }\n}"): (typeof documents)["query OnboardingPage {\n  onboardingMySteps {\n    id\n    role\n    title\n    body\n    order\n    completed\n    completedAt\n  }\n  onboardingMyProgress {\n    completed\n    total\n    percentage\n  }\n}\n\nquery ManagerTeamProgress {\n  onboardingTeamProgress {\n    userId\n    name\n    role\n    completed\n    total\n    percentage\n    enrolled\n  }\n}\n\nmutation CompleteStep($stepId: ID!) {\n  onboardingSetStepComplete(stepId: $stepId) {\n    id\n    completed\n    completedAt\n  }\n}\n\nmutation IncompleteStep($stepId: ID!) {\n  onboardingSetStepIncomplete(stepId: $stepId) {\n    id\n    completed\n    completedAt\n  }\n}\n\nmutation AssignRole($input: AssignOnboardingRoleInput!) {\n  onboardingAssignRole(input: $input) {\n    userId\n    name\n    role\n    completed\n    total\n    percentage\n    enrolled\n  }\n}"];

export function graphql(source: string) {
  return (documents as any)[source] ?? {};
}

export type DocumentType<TDocumentNode extends DocumentNode<any, any>> = TDocumentNode extends DocumentNode<  infer TType,  any>  ? TType  : never;